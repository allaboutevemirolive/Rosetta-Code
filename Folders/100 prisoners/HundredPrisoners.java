...
public class HundredPrisoners {
    ...
    public static void main(String[] args) {
        ...
        System.out.printf("Optimal play success rate: %f%%%n", exec(n, p, HundredPrisoners::playOptimal));
        System.out.printf("Random play success rate: %f%%%n", exec( n, p, HundredPrisoners::playRandom));
    ...
    private static boolean playOptimal(int n) {
        ...
        prisoner: for (int i = 0; i < secretList.size(); ++i) {
            ...
            for (int j = 0; j < secretList.size() / 2; ++j) {
                // If the door to the cell he chose is opened, he will continue to the next
                // prisoner.
                if (secretList.get(prev) == i) {
                    continue prisoner;
                }
                // Otherwise, he will check the door to the cell that the cell he chose points
                // to.
                prev = secretList.get(prev);
            }
            // If the door to the cell he chose is not opened, he will continue to the next
            // prisoner.
            return false;
        }
    ...
    // Random play: Each prisoner chooses a cell at random, and the door to that
    // cell is opened.
